...
The marketing site for [outlookonthedesktop.com](https://outlookonthedesktop.com), a static
[Astro](https://astro.build) site that replaced the original Drupal site.
...
- AdSense is loaded once via `src/components/AdSense.astro` in the layout `<head>`; `ads.txt` declares the publisher ID.
- Sitemaps are generated by the `@astrojs/sitemap` integration (`/sitemap-index.xml`); `robots.txt` points to it.
- Legacy Drupal URLs are redirected via `public/_redirects` (Netlify-style).
